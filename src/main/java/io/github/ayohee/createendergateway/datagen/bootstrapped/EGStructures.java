...
public class EGStructures {
    ...
    public static void bootstrap(BootstrapContext<Structure> ctx) {
        ctx.register(ABANDONED_STATION, new JigsawStructure(
                new Structure.StructureSettings.Builder(ctx.lookup(Registries.BIOME).getOrThrow(EGTags.HAS_ABANDONED_STATIONS)).terrainAdapation(TerrainAdjustment.BEARD_THIN).build(),
